...
class RRTFamilyVisualizer:
    ...
    def initializer(self, environment, bounds, pose_start, pose_goal, object_radius, resolution, tree, path):
        ...
        self.goal_region = list(shapely.geometry.Point(
            pose_goal).buffer(object_radius, resolution).exterior.coords)
        self.tree = tree
        self.path = path

    def plot(self, environment, bounds, pose_start, pose_goal, object_radius, resolution, tree, path):
        self.initializer(environment, bounds, pose_start,
                         pose_goal, object_radius, resolution, tree, path)
    ...
    def parser_boundary(self):
        ...
        boundary = [(x_min, y_min), (x_min, y_max),
